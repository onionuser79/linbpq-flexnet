# LinBPQ FlexNet Integration (v2.1.27)
...
- `PC/Flexnet` over AXUDP — verified fully working against
  ...
  is closed in **v2.1.27** by rate-limiting outbound CE link-time
...
```
BPQBOL:IW2OHX-13} Version 6.0.25.28 (64 bit) and FlexNet v2.1.27
```
...
`FlexNet v2.1.27`) so you can confirm what's running.
...
- **PC/Flexnet AXIP cost-ring cycles every ~3 hours.** With
  v2.1.27 deployed against PC/Flexnet V4.0, our peer's `L *` row
