			...
			else
			{
				...
				if (len == ntohs(iphdrptr->total_len))
				{
					...
					if (crc == 0xf0b8)		// Good CRC
					{
						...
						if (len > MAXDATA)
						{
							Debugprintf("BPQAXIP Invalid Msg Len=%d Source=%s",len,inet_ntoa(RXaddr.rxaddr.sin_addr));
							return 0;
						}
					...
					Debugprintf("BPQAXIP Invalid CRC=%d Source=%s",crc,inet_ntoa(RXaddr.rxaddr.sin_addr));
		...
		for (i=0;i<PORT->NumberofUDPPorts;i++)
		{
			...
			else
			{
				...
				if (crc == 0xf0b8)		// Good CRC
				{
					...
					if (len > MAXDATA)
					{
						Debugprintf("BPQAXIP Invalid Msg Len=%d Source=%s Port %d",len,inet_ntoa(RXaddr.rxaddr.sin_addr),PORT->udpport[i]);
						return 0;
					}
