# linbpq-flexnet — Release history (archive)

Everything below was the back half of `ROADMAP.md` until 2026-09-23. It is
kept **verbatim** because the release narratives carry root causes, measured
numbers and reverted experiments that are not written down anywhere else —
but it is history, not plan. `ROADMAP.md` is the current document; this file
is only for "why was it done that way, and what had we already tried?".

Read it with two caveats:

- **Some conclusions in here were later overturned.** The most important:
  the "PC/Flexnet 60 s evaluation tick" was our own telnet-poll freeze, and
  the `-12` teardown mechanism found in v2.2.1 was not the mechanism at all
  — v2.2.2 settled it (`research/link_stability_2026-09-22/`). Where a
  section contradicts `CLAUDE.md`, `CLAUDE.md` wins.
- **Current state statements are frozen at their date.** "Current state:
  v2.2.1", "production is a leaf", "`-4` peers with us" were all true when
